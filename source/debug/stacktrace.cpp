...
#
#elif TOOLKIT_PLATFORM_TYPE != TOOLKIT_PLATFORM_MIPS
#
#  include <memory>
#  include <cstdint>
#
...
namespace tinyToolkit
{
	namespace debug
	{
		 ...
		 *
		 */
		static void WINAPI ResolveSymbol(HANDLE handle, std::string & content)
		{
			CONTEXT context{ 0 };
	...
	#elif TOOLKIT_PLATFORM_TYPE == TOOLKIT_PLATFORM_APPLE || TOOLKIT_PLATFORM_TYPE == TOOLKIT_PLATFORM_LINUX
		 ...
		 *
		 */
		static std::string ResolveSymbol(const std::string & symbol)
		{
			std::size_t pos = 0;
			std::size_t head;
			std::size_t tail;
