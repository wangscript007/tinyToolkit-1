...
namespace tinyToolkit
{
	namespace options
	{
		 ...
		 *
		 */
		void Options::Parse(int argc, const char * argv[], const char * separator)
		{
		 ...
		 *
		 */
		void Options::AddDescriptionGroup(const std::shared_ptr<DescriptionGroup> & group)
		{
		 ...
		 *
		 */
		bool Options::Exits(const std::string & option) const
		{
		...
		std::string Options::Verbose()
		{
			...
			for (auto & group : _groups)
			{
				...
				for (auto & description : group->Descriptions())
				{
					width = _optionWidth - description->OptionName().length();
