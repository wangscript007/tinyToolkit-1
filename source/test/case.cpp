...
namespace tinyToolkit
{
	namespace test
	{
		...
		void Case::Run()
		{
			...
			{
				Console::Instance().Info(Message() << "[ RUN      ] " << Name() << "." << test->Name() << TOOLKIT_LINE_EOL);
				...
				if (test->IsPassed())
				{
					...
					Console::Instance().Info(Message() << "[       OK ] " << Name() << "." << test->Name() << " (" << used << " ms)" << TOOLKIT_LINE_EOL);
				}
				else
				{
					_failedList.push_back(std::string(Name()).append(".").append(test->Name()));

					Console::Instance().Fatal(Message() << "[  FAILED  ] " << Name() << "." << test->Name() << " (" << used << " ms)" << TOOLKIT_LINE_EOL);
				}
